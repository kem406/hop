...
# PART 3: SIMULATED DATA PLOT PRE-OBD
plot0_y=np.array([np.sum(imageio.imread(y_fname_nocurve_preobd(0,1)))])
plot0_x=np.array([0])
for i in range (1,101):
    j=1; # perhaps try mean of j
    #print(np.sum(imageio.imread(gt_y_fname(i))))
    plot0_y=np.concatenate((plot0_y, np.array([np.sum(imageio.imread(y_fname_preobd(i,j)))])), axis=0)
    plot0_x=np.concatenate((plot0_x, np.array([i])), axis=0)
#print(plot0_x)
#print(plot0_y)
plt.scatter(plot0_x, plot0_y)

# PART 4: NO TRANSIT GROUND TRUTH PLOT
...
# PART 5: NO TRANSIT SIMULATED DATA PLOT
plot0_x=np.array([0])
...
# PART 6: NO TRANSIT PRE-OBD NON GT PLOT
plot0_y=np.array([np.sum(imageio.imread(y_fname_nocurve_preobd(0,1)))])
plot0_x=np.array([0])
for i in range (1,100):
    j=1; # perhaps try mean of j
